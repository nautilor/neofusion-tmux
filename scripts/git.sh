...
IFS=' ' read -r -a hide_status <<< $(get_tmux_option "@neofusion-git-disable-status" "false")
IFS=' ' read -r -a current_symbol <<< $(get_tmux_option "@neofusion-git-show-current-symbol" "✓")
IFS=' ' read -r -a diff_symbol <<< $(get_tmux_option "@neofusion-git-show-diff-symbol" "!")
IFS=' ' read -r -a no_repo_message <<< $(get_tmux_option "@neofusion-git-no-repo-message" "")
IFS=' ' read -r -a no_untracked_files <<< $(get_tmux_option "@neofusion-git-no-untracked-files" "false")
IFS=' ' read -r -a show_remote_status <<< $(get_tmux_option "@neofusion-git-show-remote-status" "false")
