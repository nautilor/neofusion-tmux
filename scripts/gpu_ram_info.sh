...
get_platform()
{
  case $(uname -s) in
    Linux)
      ...
      gpu_label=$(get_tmux_option "@neofusion-force-gpu" false)
...
{
  gpu=$(get_platform)
  gpu_vram_percent=$(get_tmux_option "@neofusion-gpu-vram-percent" false)
  if [[ "$gpu" == NVIDIA ]]; then
    ...
    exit 0
      ...
      used_accuracy=$(get_tmux_option "@neofusion-gpu-vram-used-accuracy" "d")
      total_accuracy=$(get_tmux_option "@neofusion-gpu-vram-total-accuracy" "d")
...
main()
{
  ...
  RATE=$(get_tmux_option "@neofusion-refresh-rate" 5)
  gpu_label=$(get_tmux_option "@neofusion-gpu-vram-label" "VRAM")
  gpu_usage=$(get_gpu)
